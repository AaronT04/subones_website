...
router.post("/api/bone_metrics/:bone_id", async (req, res) => {
  try {
    const { bone_id } = req.params;
    ...
    res.json({
      success: true,
      bone_id: Number(bone_id),
      rows_saved: values.length,
    });
  } catch (err) {
    console.error("Error saving bone metrics by bone_id:", err);
    res.status(500).json({ error: err.message });
  }
});
