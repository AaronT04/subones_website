...
export default function DentalTable({ dentition }) {
  ...
  const getToothRecord = (tooth_name: string) => {
    return api?.dental_inventory?.find((t) => t.tooth_name === tooth_name) || null;
  };

  // Generic per-field update handler
  const handleChange = (tooth_name: string, field: string, rawValue: string) => {
    const value = rawValue === "" ? null : Number(rawValue);
    updateField("dental_inventory", { tooth_name, [field]: value }, "tooth_name");
  };
